  ...
  return (
    <div className={`mt-20 px-4 sm:px-16 lg:px-32 h-[89vh] w-full flex justify-center items-center ${theme === "dark" ? "bg-zinc-900" : "bg-zinc-100"}`}>
      <div className={`lg:w-[32rem] sm:w-[28rem] w-[20rem] ${theme === "dark" ? "bg-[#3C3C3C]" : "bg-[#C3C3C3]"} px-10 py-4 rounded-lg ${theme === "dark" ? "border-gray-600" : "border-gray-400"} border-[.5px] flex flex-col items-center`}>
        ...
        <form onSubmit={(e) => { handleSignUp(e) }} action="" className="flex flex-col gap-3 w-full">
          <input onChange={(e) => { handleInputChange(e) }} value={inputData.name} type="text" name="name" id="name" placeholder="Enter Full Name" className={`${theme === "dark" ? "bg-zinc-900 text-white" : "bg-zinc-100 text-black"} pl-2 py-2 w-full rounded-lg text-md outline outline-1 outline-[#3E5FB5]`} />
          ...
          <input onChange={(e) => { handleInputChange(e) }} value={inputData.confirmPassword} type="password" name="confirmPassword" id="confirmPassword" placeholder="Confirm Password" className={`${theme === "dark" ? "bg-zinc-900 text-white" : "bg-zinc-100 text-black"} pl-2 py-2 w-full rounded-lg text-md outline outline-1 outline-[#3E5FB5]`} />
        ...
        <p className={`${theme === "dark" ? "text-gray-200" : "text-gray-700"} text-sm mt-2`}>Have an account <Link to={'/login'} className="text-base text-[#3E5FB5] font-semibold cursor-pointer">Sign In</Link></p>
      </div>
    </div>
  )
}
